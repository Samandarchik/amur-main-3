...
export function CartSheet({ open, onOpenChange }: CartSheetProps) {
  ...
  const fixImageUrl = (url: string) => {
    ...
    if (url.includes('localhost') || url.includes('127.0.0.1')) {
      ...
      const fixed = url.replace(/https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?/g, 'http://https://backend.amur1.uz')
    ...
    if (url.startsWith('/uploads/')) {
      const fixed = `http://https://backend.amur1.uz${url}`
    ...
    if (url.startsWith('http://https://backend.amur1.uz')) {
    ...
    if (!url.startsWith('http')) {
      const fixed = `http://https://backend.amur1.uz/${url.replace(/^\/+/, '')}`
